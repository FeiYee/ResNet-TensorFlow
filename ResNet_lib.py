...
def conv2d_same(input_tensor,num_outputs,kernel_size,stride,is_train = True,activation_fn=tf.nn.relu,normalizer_fn = True):
    data = input_tensor
    if stride is 1:
        data = slim.conv2d(inputs = data,num_outputs = num_outputs,kernel_size = kernel_size,stride = stride,weights_regularizer=slim.l2_regularizer(WEIGHT_DECAY),activation_fn=None,padding='SAME')
    else:
        ...
        data = slim.conv2d(inputs = data,num_outputs = num_outputs,kernel_size = kernel_size,stride = stride,weights_regularizer=slim.l2_regularizer(WEIGHT_DECAY),activation_fn=None,padding='VALID')
...
def bottleneck(input_tensor,output_depth,stride,is_train):
    ...
    #降通道处理
    data = conv2d_same(data,output_depth//4,1,1,is_train)
    #提取特征
    data = conv2d_same(data,output_depth//4,3,stride,is_train)
    #通道还原
    data = conv2d_same(data,output_depth,1,1,is_train,None,normalizer_fn = False)
    #生成残差
